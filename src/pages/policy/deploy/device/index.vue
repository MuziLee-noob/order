      ...
      <Row class="check-condition clearfix">
        <Form :label-width="80">
          ...
          <FormItem label="策略内容" class="mgr" v-if="scoped">
            <Select v-model="policyDetailCode" clearable @on-change="deviceList(1)">
          ...
          <div class="btns">
            <Button class="search apply" @click="deviceList(1)">查询</Button>
            <Button class="reset" @click="reset">重置</Button>
            <span class="refurbish iconfont icon-refresh" @click="deviceList(1)"></span>
          </div>
        </Form>
      </Row>
    </div>
...
export default {
  ...
  computed: {
    limitAdd() {
      let limit = false
      this.deviceData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_DEVICE_CREATE') {
          limit = true
        }
      })
      return limit
    ...
    limitManage() {
      let limit = false
      this.deviceData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_DEVICE_EDIT') {
          limit = true
        }
      })
      return limit
    ...
    limitStop() {
      let limit = false
      this.deviceData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_DEVICE_STATUS') {
          limit = true
        }
      })
      return limit
    ...
    limitDelete() {
      let limit = false
      this.deviceData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_DEVICE_DELETE') {
          limit = true
        }
      })
      return limit
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          if (this.userTitle === '创建策略') {
            this.formValidate.policyTypeCode = 'device'
