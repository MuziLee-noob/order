    ...
    >
      <Form :label-width="165" :model="formValidate" :rules="ruleValidate" ref="formValidate">
        <FormItem label="用户登录失败次数上限" prop="configValue">
          ...
          <Select v-model="unitCode" clearable>
...
export default {
  ...
  computed: {
    limitManage() {
      ...
      this.highData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_CONFIG_EDIT') {
          limit = true
        }
      })
      return limit
    ...
    limitStop() {
      let limit = false
      this.highData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_CONFIG_STATUS') {
          limit = true
        }
      })
      return limit
    ...
    limitDelete() {
      let limit = false
      this.highData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_CONFIG_DELETE') {
          limit = true
        }
      })
      return limit
  ...
  methods: {
    ...
    initSelect(id) {
      ...
      highConfgiSelect(params).then(res => {
        if (res.status === 0) {
    ...
    swithClick: function(id, type) {
      ...
      highConfigSwith(params).then(res => {
        if (res.status === 0) {
          this.highList()
          this.delModel = false
        }
