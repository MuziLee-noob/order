      ...
      <Row class="check-condition clearfix">
        <Form :label-width="80">
          ...
          <FormItem label="策略内容" class="mgr" v-if="scoped">
            <Select v-model="policyDetailCode" @on-change="userList(1)" clearable>
          ...
          <div class="btns">
            <Button @click="userList(1)" class="search apply">查询</Button>
            <Button @click="reset" class="reset">重置</Button>
            <span @click="userList(1)" class="refurbish iconfont icon-refresh"></span>
          </div>
        </Form>
      </Row>
    </div>
...
export default {
  ...
  computed: {
    limitAdd() {
      let limit = false
      this.userData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_USER_CREATE') {
          limit = true
        }
      })
      return limit
    ...
    limitManage() {
      let limit = false
      this.userData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_USER_EDIT') {
          limit = true
        }
      })
      return limit
    ...
    limitStop() {
      let limit = false
      this.userData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_USER_STATUS') {
          limit = true
        }
      })
      return limit
    ...
    limitDelete() {
      let limit = false
      this.userData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_USER_DELETE') {
          limit = true
        }
      })
      return limit
  ...
  created() {
    ...
    this.userList()
    this.initopId()
    this.formValidate.policyValidityTimeUnit = this.policyTimes[0].value
  },
  methods: {
    // 获取当天的时间
    getToday: function() {
    ...
    userList: function(current) {
      ...
      if (this.policyScopeCode === undefined) {
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          if (this.userTitle === '创建策略') {
            this.formValidate.policyTypeCode = 'user'
