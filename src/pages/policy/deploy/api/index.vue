      ...
      <Row class="check-condition clearfix">
        <Form :label-width="80">
          ...
          <FormItem label="策略对象" class="mgr">
            <Select v-model="policyScopeCode" clearable @on-change="searchListContent">
              <Option
                v-for="item in detailCode"
                :key="item.opId"
                :value="item.opId"
              >
                {{ item.policyName }}
          ...
          <FormItem label="策略内容" class="mgr" v-if="scoped">
            <Select v-model="policyDetailCode" clearable @on-change="apiList(1)">
          ...
          <div class="btns">
            <Button @click="apiList(1)" class="search apply">查询</Button>
            <Button @click="reset" class="reset">重置</Button>
            <span @click="apiList(1)" class="refurbish iconfont icon-refresh"></span>
          </div>
        </Form>
      </Row>
    </div>
    ...
    >
      <Form :label-width="80" :model="formValidate" :rules="ruleValidate" ref="formValidate">
        ...
        <FormItem label="API类型">
          <RadioGroup v-model="policyApiType" @on-change="initDetailSelect(policyApiType)">
            <Radio v-for="item in openArray" :key="item.opId" :label="item.opId">{{item.policyName}}</Radio>
          </RadioGroup>
        </FormItem>
        ...
        <FormItem label="策略内容" prop="policyDetailCode">
          <Input type="text" v-model="formValidate.policyDetailCode" disabled />
        </FormItem>
...
export default {
  ...
  computed: {
    limitAdd() {
      let limit = false
      this.apiData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_API_CREATE') {
          limit = true
        }
      })
      return limit
    ...
    limitManage() {
      let limit = false
      this.apiData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_API_EDIT') {
          limit = true
        }
      })
      return limit
    ...
    limitStop() {
      let limit = false
      this.apiData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_API_STATUS') {
          limit = true
        }
      })
      return limit
    ...
    limitDelete() {
      let limit = false
      this.apiData.forEach((data, ind) => {
        if (data.menuCode === 'DEPLOY_API_DELETE') {
          limit = true
        }
      })
      return limit
    ...
    initObjectSelect() {
      ...
      apiConfigObject(params).then(res => {
        if (res.status === 0) {
          ...
          this.initDetailSelect(this.openArray[0].opId)
        }
      })
    },
    // 通过第一个下拉框获取第二个下拉框内容
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          if (this.userTitle === '创建策略') {
            this.formValidate.policyTypeCode = 'api'
            this.formValidate.policyApiType = this.policyApiType
            if (this.policyValidityType === '永久') {
              this.formValidate.policyValidityType = '0'
              this.formValidate.policyValidValue = ''
              this.formValidate.policyValidTimeUnit = ''
            } else {
    ...
    initCollect(id) {
      ...
      configInit(params).then(res => {
        if (res.status === 0) {
          ...
          this.policyApiType = this.collectEditData.policyApiType
